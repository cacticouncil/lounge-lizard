...
const threadsPageTemplate = handlebars.compile(fs.readFileSync(path.join(__dirname, 'chat', 'threadsPage.html')).toString())
...
handlebars.registerHelper('isFirstUnread', function(messageList, message, options) {
  if (messageList && messageList.firstUnreadTs === message.timestamp)
...
handlebars.registerHelper('isChannel', function(messageList, options) {
  if (messageList && messageList.type === 'channel')
    return options.fn(this)
  else
    return options.inverse(this)
})
