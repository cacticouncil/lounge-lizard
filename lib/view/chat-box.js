...
const threadHtml = fs.readFileSync(path.join(__dirname, 'chat', 'thread.html')).toString(); //added
const threadTemplate = handlebars.compile(threadHtml);
const threadsPageTemplate = handlebars.compile(fs.readFileSync(path.join(__dirname, 'chat', 'threadsPage.html')).toString()); //added
...
class ChatBox {
  ...
  unloadThreads(){
    console.log('unload threads called');
    if(!this.threads || !this.threadBindings)
      return;
    for(let binding of this.threadBindings)
      binding.detach();
  }
  ...
  async loadThreads(threads){
    ...
    //subscriptions, not currently working
    ...
    if (process.env.WEY_DEBUG === '1')
        fs.writeFileSync('threadsPage.html', html);
    this.loadedTimes++;
    const domain = threads[0].account.url;
    this.browser.loadHTML(html, domain);
    //console.log('Checking threads:');
    //console.log(threads[0].onMessage);
    this.replyBox.setVisible(false); //may remove, not working
    this.replyEntry.setEnabled(false);
  }

  _newMessage(message) {
    if (this.threadId && this.threadId !== message.threadId) //if new message is not part of current thread
      return
    ...
    if (this.messageList && message.timestamp === this.messageList.firstUnreadTs)
      firstUnread = true
  ...
  _newThreadMessage(message, thread){
    ...
    this.browser.executeJavaScript(`window.addMessage(${JSON.stringify(html)}, ${fromCurrentUser}, ${thread.id})`, () => {})
  }
  ...
  _sendMessage(threadId, message){
    ...
    //this.mainWindow.showThreads(this.account);
  }
