...
class SlackAccount extends Account {
  ...
  setupRTMClient(token) {
    ...
    this.rtm.on('conversations.open', this.joinDMAsync.bind(this))
  ...
  async joinDM(userId) {
    let direct_message = await this.joinDMAsync(userId)
    return direct_message.channel.id
  }

  async joinDMAsync(us) {
  ...
  async getChannelMembers(channelId) {
    //DEPRECATED--> const slackChannel = (await this.rtm.webClient.channels.info({channel: channelId})).channel
    const slackChannel = (await this.rtm.webClient.conversations.info({channel: channelId})).channel
    const userList = slackChannel.members.map(u => this.users[u])
    return userList
  }

  async getUserPresence(userId) {
    let presence = await this.rtm.webClient.users.getPresence({user: userId})
    return presence.presence ? presence.presence : null
  }
