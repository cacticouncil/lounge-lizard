...
class TeamsAccount extends Account {
  ...
  /**
   * The previous implementation used apiHelper (a Graph-based module). This getter exists
   * to easily identify leftover uses of apiHelper.
   */
  get apiHelper () {
    throw new Error('apiHelper is no longer available')
  }
  ...
  /**
   * Event handler for team updates
   * @param {any} team API response for team
   */
  onUpdateTeam(team) {
    this.id = team.id
  ...
  /**
   * This function is from the preivous implementation based on the Graph API.
   * I commented this out instead of deleting because it contains some Lounge Lizard patterns
   * we should follow when implementing our version.
   */
