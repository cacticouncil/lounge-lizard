...
const { handler: newMessageHandler } = require('./listeners/new-message-listener')
...
class MicrosoftAccount extends MSTeamsClient {
  ...
  static findOrCreate(options = {}) {
    ...
    if (!this.cache[key]) {
      ...
      // register events here on a per-account basis, instead of a per-team basis
      msa.on('new-message', newMessageHandler)
      msa.poller.start()
      this.cache[key] = msa
    } else {
